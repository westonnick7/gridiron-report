...
function InjCol({ code, inj }) {
  ...
  return (
    <div className="injcol">
      ...
      <div className="injrow"><span>Key Injuries</span>{!key.length && <b style={{ color: "var(--muted)" }}>None reported</b>}</div>
      {key.length > 0 && (
        <div className="injlist">
          {key.map((k, i) => {
            const out = k.status === "Out" || k.status === "Doubtful";
            return (
              <div className="injlrow" key={i}>
                <span><span className="injnm">{k.name || k.pos}</span><span className="injpos">{k.pos}{k.injury ? " · " + k.injury : ""}</span></span>
                <span className={"injst " + (out ? "out" : "q")}>{k.status}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="qmodal">
      <div className="qhead">
        ...
        {bx ? (
          <div className="qscore">
            <span className={"qsc" + (aW ? " w" : "")}>{bx.awayScore}</span>
            <span className="qfin">Final</span>
            <span className={"qsc" + (hW ? " w" : "")}>{bx.homeScore}</span>
          </div>
        ) : (
          <>
      ...
      <div className="qbody">
        {bx && (
          <>
            <div className="qsub">Box Score</div>
            <div className="qrow"><div className="qv l">{bx.awayScore}</div><div className="qk">Final Score</div><div className="qv r">{bx.homeScore}</div></div>
            <div className="qrow"><div className="qv l">{bx.awayYds != null ? bx.awayYds : DASH}</div><div className="qk">Total Yds</div><div className="qv r">{bx.homeYds != null ? bx.homeYds : DASH}</div></div>
            <div className="qrow"><div className="qv l">{bx.awayTo != null ? bx.awayTo : DASH}</div><div className="qk">Turnovers</div><div className="qv r">{bx.homeTo != null ? bx.homeTo : DASH}</div></div>
            <div className="qsub">Season Averages</div>
          </>
        )}
        {QUICK.map((m) => (
...
function stWins(r) { const m = /^(\d+)-(\d+)/.exec(r || ""); return m ? parseInt(m[1], 10) : 0; }
function stLoss(r) { const m = /^(\d+)-(\d+)/.exec(r || ""); return m ? parseInt(m[2], 10) : 0; }
function stSort(codes, ts) {
  return codes.slice().sort((a, b) => {
    const ra = record(ts, a), rb = record(ts, b);
    const wa = stWins(ra), wb = stWins(rb);
    if (wb !== wa) return wb - wa;
    const la = stLoss(ra), lb = stLoss(rb);
    if (la !== lb) return la - lb;
    return (ts[b].ppg || 0) - (ts[a].ppg || 0);
  });
}
function StTable({ codes, ts, full }) {
  return (
    <div className="stbl-wrap"><table className="stbl">
      <thead><tr><th className="lft">Team</th><th>Rec</th><th>PPG</th><th>PAPG</th>{full && <th>Off EPA</th>}{full && <th>Def EPA</th>}</tr></thead>
      <tbody>{codes.map((c) => {
        const s = ts[c];
        return (
          <tr key={c}>
            ...
            <td>{s.record}</td><td>{f1(s.ppg)}</td><td>{f1(s.papg)}</td>
            {full && <td>{f2(s.offEpa)}</td>}{full && <td>{f2(s.defEpa)}</td>}
          </tr>
        );
      })}</tbody>
    </table></div>
...
function StandingsView({ data }) {
  ...
  const league = codes.filter((c) => DIVISION[c]);
  const CONFS = ["AFC", "NFC"];
  const DIVS = ["East", "North", "South", "West"];
  const modes = [["overall", "League"], ["conference", "Conference"], ["division", "Division"]];
  return (
    <>
      <div className="stmode">{modes.map(([m, l]) => (
        <button key={m} className={mode === m ? "on" : ""} onClick={() => setMode(m)}>{l}</button>
      ))}</div>
      {mode === "overall" && <StTable codes={stSort(codes, ts)} ts={ts} full />}
      {mode === "conference" && (
        <div className="stgrid">{CONFS.map((cf) => (
          <div className="stgrp" key={cf}>
            <div className="grouphd">{cf}</div>
            <StTable codes={stSort(league.filter((c) => DIVISION[c][0] === cf), ts)} ts={ts} />
          </div>
        ))}</div>
      )}
      {mode === "division" && (
        <div className="stgrid">{CONFS.flatMap((cf) => DIVS.map((dv) => (
          <div className="stgrp" key={cf + dv}>
            <div className="grouphd">{cf} {dv}</div>
            <StTable codes={stSort(league.filter((c) => DIVISION[c][0] === cf && DIVISION[c][1] === dv), ts)} ts={ts} />
          </div>
        )))}</div>
      )}
    </>
          ...
          {weekGames.length ? (
            <div className="grid">
              {weekGames.map((g, i) => {
                ...
                const bx = g.box || null;
                const aSc = bx ? bx.awayScore : null, hSc = bx ? bx.homeScore : null;
                const aWin = bx && aSc > hSc, hWin = bx && hSc > aSc;
                const hasStats = bx && (bx.awayYds != null || bx.homeYds != null);
                return (
                  <div className={"game" + (bx ? " final" : "")} key={i} onClick={(e) => { if (e.target.closest(".cta")) { e.preventDefault(); setModal({ type: "quick", away: a, home: h, week: g.week }); } else setModal({ type: "game", away: a, home: h, week: g.week }); }}>
                    <div className="spine2"><span style={{ background: ca }} /><span style={{ background: ch }} /></div>
                    <div className="ghead"><span>{g.divisional ? "Division" : "Inter-conf"}</span>{bx ? <span className="kick fin">Final</span> : <span className="kick">{fmtKick(g.kickoff)}</span>}</div>
                    <div className={"grow" + (bx && !aWin ? " dim" : "")}><Badge ab={a} cls="gbadge" /><div className="gteam"><div className="gn">{nick(a)}</div><div className="gc">{a} · Away</div></div>{bx ? <span className={"gsc tnum" + (aWin ? " w" : "")}>{aSc}</span> : <span className="rec tnum">{record(data.teamStats, a)}</span>}</div>
                    <div className={"grow" + (bx && !hWin ? " dim" : "")}><Badge ab={h} cls="gbadge" /><div className="gteam"><div className="gn">{nick(h)}</div><div className="gc">{h} · Home</div></div>{bx ? <span className={"gsc tnum" + (hWin ? " w" : "")}>{hSc}</span> : <span className="rec tnum">{record(data.teamStats, h)}</span>}</div>
                    {hasStats && (
                      <div className="boxln">
                        <span className="boxlbl">Total Yds</span><span className="tnum">{bx.awayYds != null ? bx.awayYds : DASH}</span><span className="tnum">{bx.homeYds != null ? bx.homeYds : DASH}</span>
                        <span className="boxlbl">Turnovers</span><span className="tnum">{bx.awayTo != null ? bx.awayTo : DASH}</span><span className="tnum">{bx.homeTo != null ? bx.homeTo : DASH}</span>
                      </div>
                    )}
                    <div className="gfoot">
                      {g.divisional && <span className="vchip div">Division</span>}
                      {!bx && g.odds && g.odds.detail && <span className="vchip">{g.odds.detail}</span>}
                      {!bx && g.odds && g.odds.total != null && <span className="vchip">O/U {g.odds.total}</span>}
                      ...
                      <a className="cta" href="#" onClick={(e) => e.preventDefault()}>{bx ? "Box score" : "Preview"} <ChevR /></a>
                    </div>
                  </div>
                );
              })}
      ...
      {view === "teams" && (
        <section className="wrap viewfade" key="teams">
          ...
          {["AFC", "NFC"].map((cf) => {
            ...
            return (
              <div className="confgrp" key={cf}>
                <div className="confhd">{cf} <span className="n">{codes.length} teams</span></div>
                <div className="tgrid">
                  {codes.map((c) => (
